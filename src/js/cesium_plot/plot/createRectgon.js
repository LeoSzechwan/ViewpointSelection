...
class CreateRectgon extends BasePlot {
	...
	start(callBack) {
		...
		this.handler.setInputAction(function (evt) { //单击开始绘制
			...
			if (that.positions.length == 3) { // 绘制第三个点 结束
		...
		let polygonObj = {
			polygon: {
				hierarchy: new Cesium.CallbackProperty(function () {
					let rps = that.computePositions(that.positions);
					return new Cesium.PolygonHierarchy(rps)
				}, false),
	...
	computePositions() {
		...
		let p3 = this.positions[2];



		let mid = Cesium.Cartesian3.midpoint(p1.clone(), p2.clone(), new Cesium.Cartesian3());
		const radius = Cesium.Cartesian3.distance(p1.clone(), p2.clone()) / 2;
		let dir = Cesium.Cartesian3.subtract(p3.clone(), mid.clone(), new Cesium.Cartesian3());
		dir = Cesium.Cartesian3.normalize(dir, new Cesium.Cartesian3());

		let newp3 = Cesium.Cartesian3.multiplyByScalar(dir.clone(), radius, new Cesium.Cartesian3());
		newp3 = Cesium.Cartesian3.add(newp3.clone(), mid.clone());

		let negate = Cesium.Cartesian3.negate(dir.clone(), new Cesium.Cartesian3());
		negate = Cesium.Cartesian3.multiplyByScalar(negate.clone(), radius, new Cesium.Cartesian3());
		p4 = Cesium.Cartesian3.add(negate.clone(), mid.clone());
		return [p1.clone(), newp3.clone(), p2.clone(), p4.clone()];
	}
