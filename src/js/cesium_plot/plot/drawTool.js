 ...
 */
  ...
  */
  ...
  /**
   * 由geojson格式数据FeatureCollection创建多个对象
   * @param {Object} data geojson格式数据-FeatureCollection
  */
  allCreateByGeojson(data) {
    let { features } = data;
    let entObjArr = [];
    for (let i = 0; i < features.length; i++) {
      let entObj = this.createByGeojson(features[i])
  ...
  /**
    * 由geojson格式数据Feature创建单个对象
    * @param {Object} data geojson格式数据-Feature
   */
  ...
  allToGeojson() {
    ...
    for (let i = 0; i < this.entityObjArr.length; i++) {
      featureCollection.features.push(this.toGeojson(this.entityObjArr[i]));
    }
  ...
  /**
   * 单个要素转为Geojson
   * @returns {Object} feature geojson格式数据
   */
  toGeojson(item) {
    if (!item) return;
    ...
    let coordinates = [];
    if (item.type == 'point') {
      coordinates = [lnglats[0], lnglats[1]]
    } else {
  ...
  */
  ...
  createDelteDom(px, objId) {
    ...
    deleteDom.style.position = "fixed";
